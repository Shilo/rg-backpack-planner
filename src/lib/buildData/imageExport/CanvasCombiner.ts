...
export class CanvasCombiner {
  ...
  async combine(
    ...
    try {
      if (blobs.length !== 3) {
        return { success: false, error: `Expected 3 blobs, got ${blobs.length}` };
      }
      ...
      for (let i = 0; i < images.length; i++) {
        if (!images[i]) {
          return { success: false, error: `Failed to load image ${i}` };
        }
      }
      ...
      const totalHeight = treeHeights.reduce((sum, h) => sum + h, 0) + (gap * 2);
      ...
      if (!resultBlob) {
        return { success: false, error: 'Failed to convert combined canvas to blob' };
      }

      return { success: true, data: resultBlob };
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error';
      return { success: false, error: `Canvas combination failed: ${message}` };
    }
  }
