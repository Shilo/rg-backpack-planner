...
export class TreeImageCapturer {
   ...
   */
  async capture(treeId: "guardian" | "vanguard" | "cannon"): Promise<Result<Blob>> {
    try {
      ...
      const viewportEl = document.querySelector('.tree-viewport') as HTMLElement;
      if (!viewportEl) {
        return { success: false, error: 'Tree viewport element not found' };
      }
      ...
      if (!blob) {
        return { success: false, error: 'Failed to convert canvas to blob' };
      }
      
      return { success: true, data: blob };
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error';
      return { success: false, error: `Canvas rendering failed: ${message}` };
    }
  }
}
