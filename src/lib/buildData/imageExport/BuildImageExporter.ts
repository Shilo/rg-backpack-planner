...
export class BuildImageExporter {
   ...
   */
  async export(buildName?: string): Promise<Result<"clipboard" | "download">> {
    // Save current active tab state
    const originalTabId = get(activeTabId);

    try {
      // Capture all three trees
      const treeIds: Array<"guardian" | "vanguard" | "cannon"> = ["guardian", "vanguard", "cannon"];
      const blobs: Blob[] = [];
      ...
      return { success: false, error: `Build image export failed: ${message}` };
    } finally {
