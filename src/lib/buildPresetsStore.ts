...
function defaultPresetsData(): BuildPresetsData {
  ...
  return {
    ...
    presets: [
      { id: "default", name: "Default", encoded: emptyEncoded },
    ],
  };
}
...
function validatePresetsData(raw: unknown): BuildPresetsData | null {
  ...
  if (typeof activePresetId !== "string" || !Array.isArray(presets)) return null;
  if (presets.length === 0) return null;
  const list: BuildPreset[] = [];
  for (const p of presets) {
    if (!p || typeof p !== "object") return null;
    const q = p as Record<string, unknown>;
    if (typeof q.id !== "string" || typeof q.name !== "string" || typeof q.encoded !== "string")
      return null;
...
export function savePresetsToStorage(data: BuildPresetsData): void {
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === "QuotaExceededError") {
      console.warn("localStorage quota exceeded, unable to save build presets");
    } else {
      console.error("Failed to save build presets to localStorage:", error);
    }
  }
}
  ...
  buildPresetsStore.update((data) => ({
    ...
    presets: data.presets.map((p) => {
      ...
      return {
        ...
        ...(updates.encoded !== undefined && { encoded: updates.encoded }),
      };
    }),
  }));
}
...
export function deletePreset(id: string): void {
  buildPresetsStore.update((data) => {
    ...
    if (presets.length === 0) return data;
